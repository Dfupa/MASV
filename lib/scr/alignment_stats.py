...
class Bamstats(object):
    def __init__(self, bam):
        ...
        self.mapped_percent = round(self.mapped / self.total, ndigits=2)
...
def main():
    ...
    with open(args.output, 'w') as output:
        ...
        output.write("Mapped_fraction:\t{0}\t{1}\n".format(round(sum([s.mapped for s in stats]) / sum([s.total for s in stats]), ndigits=3), '\t'.join([str(s.mapped_percent) for s in stats])))
